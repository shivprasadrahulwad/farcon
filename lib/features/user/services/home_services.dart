...
import 'package:farcon/models/ratings.dart';
...
/// fetch all farmers by address
  static Future<void> fetchShopsByAddress(
    BuildContext context, String address) async {
  final shopDetailsProvider =
      Provider.of<ShopDetailsProvider>(context, listen: false);

  try {
    print('🔍 Fetching shop details for address: $address');

    final response = await http.get(
      Uri.parse('$uri/api/shop-info/address/$address'),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData is List && responseData.isNotEmpty) {
        List<ShopDetails> shops = responseData.map((shopData) {
          final Map<String, dynamic> mappedData = Map<String, dynamic>.from(shopData);
          return ShopDetails.fromMap(mappedData);
        }).toList();

        print('🔄 Updating ShopDetailsProvider with fetched shops');
        shopDetailsProvider.setShopList(shops);
        ...
        print('⚠️ Response data is not valid or empty');
        throw Exception('No shops found for the provided address');
      }
    } else {
    ...
    print('💥 Error in fetchShopsByAddress: $e');
